...
For each SDK entry, this script:
  1. Runs the API parser to produce a YAML intermediate
  2. Runs the API generator to produce Hugo-compatible markdown

Expects a shared .venv to already exist (created by api_venv_setup.py).
When SKIP_VENV_SETUP=true, uses the current Python instead of the venv.
"""
